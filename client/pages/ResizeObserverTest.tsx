...
export default function ResizeObserverTest() {
  const [currentTest, setCurrentTest] = useState<"enhanced" | "basic">(
    "enhanced",
  );
  ...
  const testProblem = {
    ...
    description:
      "A test problem to verify ResizeObserver error handling in Monaco Editor components.",
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h1 className="text-3xl font-bold">
                    ResizeObserver Test Page
                  </h1>
                  <p className="text-muted-foreground">
                    Test and verify ResizeObserver error handling fixes for
                    Monaco Editor
                  </p>
                </div>
              </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t">
              <div className="space-y-2">
                <label className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    Show Second Editor (Multi-editor test)
                  </span>
                </label>
              </div>
              ...
              <div className="text-sm text-muted-foreground">
                <p>
                  <strong>Current Size:</strong> {containerSize} (
                  {containerStyles[containerSize].width} ×{" "}
                  {containerStyles[containerSize].height})
                </p>
                <p>
                  <strong>Status:</strong>{" "}
                  {rapidResize ? "Rapid resizing active" : "Ready for testing"}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3 text-sm">
              <div>
                <strong>1. Container Resizing:</strong> Use the size buttons to
                test how the editor handles container size changes.
              </div>
              <div>
                <strong>2. Rapid Resize Test:</strong> Click "Rapid Resize" to
                test the debouncing and error handling during rapid size
                changes.
              </div>
              <div>
                <strong>3. Multi-Editor Test:</strong> Enable "Show Second
                Editor" to test multiple Monaco instances.
              </div>
              <div>
                <strong>4. Collapse/Expand:</strong> Test how the editor handles
                visibility changes.
              </div>
              <div>
                <strong>5. Console Check:</strong> Open browser console to
                verify no ResizeObserver errors are logged.
              </div>
              <div className="mt-4 p-3 bg-primary/5 rounded">
                <strong>Expected Result:</strong> No "ResizeObserver loop limit
                exceeded" errors should appear in the console, and the editor
                should resize smoothly without layout issues.
              </div>
            </div>
          </CardContent>
        </Card>
